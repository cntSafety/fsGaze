...
export const useProviderPortFailures = (swComponentUuid: string) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const portsResult = await getProviderPortsForSWComponent(swComponentUuid);
      if (!portsResult.success || !portsResult.data) {
        throw new Error(portsResult.message || 'Failed to fetch provider ports.');
      }
      setProviderPorts(portsResult.data);
    ...
    try {
      ...
      if (result.success && result.failureUuid) {
        const newFailure: PortFailure = {
          ...
          asil: values.asil,
          failureType: null, // This might need adjustment based on data model
          relationshipType: 'OCCURRENCE', // Or other relevant type
        };
