...
export async function getSafetyNodesForComponent(componentUuid: string): Promise<{
    ...
    try {
        ...
        const componentSafetyData: ComponentSafetyData[] = result.records.map((record: Record) => ({
            ...
            Severity: record.get('Severity') ? record.get('Severity').toNumber() : null,
            Occurrence: record.get('Occurrence') ? record.get('Occurrence').toNumber() : null,
            Detection: record.get('Detection') ? record.get('Detection').toNumber() : null,
