...
export async function GET(request: NextRequest, { params }: { params: Promise<{ uuid: string }> }) {
  try {
    ...
    const result = await getFailuresForPorts(uuid);
    ...
    return NextResponse.json({ success: false, message: `Error getting failure modes for port: ${errorMessage}` }, { status: 500 });
  }
} 